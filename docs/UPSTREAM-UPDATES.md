...
客户端和服务端分别使用独立的 GitHub Actions 工作流，都只在手动点击 **Run workflow** 时运行。构建所选 Git 分支的本项目代码，并使用本次输入选择上游协议核心。全部构建与校验成功后，自动发布本次二进制、对应源码及 `SHA256SUMS.txt` 到 Releases；每次运行或重跑使用独立标签，以预发布版提供。
...
服务端包内的 24.10 升级工具接受 ocserv 1.5.0 及后续版本，并要求新包高于当前已安装版本；继续保留旧包校验、配置备份和失败回滚检查。
