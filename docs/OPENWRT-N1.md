...
更新日期：2026-09-17。服务端修复版：ocserv 1.5.0-r3、管理页 0.4.1。项目只构建 25.12 系列 APK。
...
官方 SDK 默认 25.12.5、软件包架构 aarch64_generic；OPL 的 APK 工具报告 `aarch64` 也受支持。APK、musl 和各用户态依赖必须匹配实际固件；先使用固件作者配置的软件源。**TUN 来自 OPL/Flippy 自己的内核**：本项目配方不依赖官方 SDK 的 kmod-tun，安装脚本要求现有 `/dev/net/tun` 可用。
...
2. 使用 WinSCP 或 LuCI 文件传输，把解压目录中的文件上传到 N1 的新目录 `/tmp/bulijie-vpn-041`。`install.sh`、两个 APK 和 `SHA256SUMS` 应在同一层；不要混入旧版本的 APK。
3. SSH 登录 N1，以 root 执行：

```sh
cd /tmp/bulijie-vpn-041
sh install.sh
```

脚本会校验套装所有文件、检查固件/架构/TUN、更新现有 APK 软件源索引并预演安装。如果计划安装或更换内核模块则停止。安装前把已有配置备份到 `/root/bulijie-before-install-日期时间-进程号`。

安装的两个包是 `ocserv-1.5.0-r3.apk` 和 `luci-app-ocserv-easy-0.4.1-r1.apk`。安装成功后自动移除重复的原版 LuCI 页面与其翻译包。`--allow-untrusted` 只用于这些自行构建且已经校验的本地 APK；普通依赖仍由固件现有软件源解析，不跳过依赖检查。
...
安装或证书步骤失败后可重新运行脚本，继续尚未完成的全新安装。防火墙写入、检查或重载失败时恢复安装前配置；已完成安装后再运行不会重复添加规则。存储报告兼容 OPL 的 btrfs 布局，不再因 `df /etc` 返回错误而中途退出。

## 第一次使用管理页

重新登录 LuCI，打开 **VPN → 布利杰VPN**，确认页面显示“管理页 0.4.1”。

1. 在“账号与在线用户”添加账号；新密码至少 8 个字符，输入两次后点击“保存并生效”。程序没有默认账号密码。
...
先退出/断开客户端并备份当前固件配置，再使用新套装的 `install.sh`。已有 ocserv 的账号、证书与配置按包管理器配置文件策略保留。旧页面错误保存的明文密码会转成正确哈希，密码本身不变；正常哈希不重新生成。无法识别的记录在新页面标为“需重设密码”。

此前跳过安装脚本、仅手动安装 APK 的设备，如果仍使用 `ocvpn`、`10.77.0.0/24` 和 `4443` 默认配置且没有 VPN 防火墙规则，会自动补齐默认转发。已有自定义网络会保留。更新不是固件升级，也不包含旧二进制的自动回滚；请保留原来可用的 APK 和固件备份。

如果改过自定义地址池，需同步检查 VPN 的转发/NAT。启用专用上网开关时会根据当前地址池生成相应规则；关闭时恢复之前的普通网络配置。

如果仍有加载或保存错误，在套装目录运行 `sh diagnose-n1.sh`。它输出组件版本、账号健康计数和管理代码故障位置，不输出用户名、密码、密码哈希或私钥。
