...
仓库 `Actions → 手动构建客户端和 OpenWrt 服务端 → Run workflow`，分支选 `main`，`component` 选 `all`、`client` 或 `server`。不需要仓库 Secret；没有 push、pull_request 或定时触发器。标准 `ubuntu-24.04` 运行器可用于这个公开仓库的免费构建，artifact 保留 7 天。

- 客户端：`scripts/build-client-linux.sh x64|x86` 使用 Ubuntu 的 MinGW MSVCRT 交叉工具链，静态合并协议与运行库，审计导入表，再打包原样 Wintun DLL。
- 服务端：`scripts/fetch-sdk.py 24.10|25.12` 校验并解压 `server/openwrt/sdks.json` 指定的 SDK；`server/tools/build-ocserv.sh` 同时构建 ocserv 1.5.0、原 LuCI 页、简体中文翻译与新增管理页。
...
SDK 的 feed 配置和实际使用的提交会随产物记录。更换 SDK 时一并更新 URL、SHA-256、版本与格式；使用固件作者 SDK 时，可直接在独立 Linux 目录调用 `build-ocserv.sh`。
